...
def train_smote_model(train_data_resampled, train_labels_resampled):
    ...
    disp.plot()
    plt.savefig('reports/figures/training/smote_model__log_reg_confusion_matrix.png')  # Save the plot as a file
    plt.close()  # Close the plot

    # Serialize baseline model to use for testing
    dump(smote_model, 'src/models/serialized/smote_model_logistic_regression.pkl')

    return metrics_dict



# SMOTE MODEL TRAINED WITH RANDOM FOREST CLASSIFIER
def train_smote_rf_model(train_data_resampled, train_labels_resampled):

    """
    Train a RF regression model with SMOTE applied and evaluate its performance.
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=smote_model.classes_)
    disp.plot()
    plt.savefig('reports/figures/training/smote_rf_model_confusion_matrix.png')  # Save the plot as a file
    plt.close()  # Close the plot

    # Serialize baseline model to use for testing
    dump(smote_model, 'src/models/serialized/smote_model_rf.pkl')

    return metrics_dict


def train_smote_xgb_model(train_data_resampled, train_labels_resampled):

    """
    Train a XGB regression model with SMOTE applied and evaluate its performance.
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=smote_model.classes_)
    disp.plot()
    plt.savefig('reports/figures/training/smote_model__xgb_confusion_matrix.png')  # Save the plot as a file
    plt.close()  # Close the plot

    # Serialize baseline model to use for testing
    dump(smote_model, 'src/models/serialized/smote_model_xgb_regression.pkl')
...
if __name__ == "__main__":
    ...
    smote_model_results = train_smote_xgb_model(train_data_resampled, train_labels_resampled)
